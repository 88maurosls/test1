...
def main():
    ...
    if 'perform_search' not in st.session_state:
        st.session_state.perform_search = False

    def on_text_change():
        st.session_state.perform_search = True

    barcode_input = st.text_input('Inserire il barcode', key='barcode_input', on_change=on_text_change)

    if st.button('Check'):
        st.session_state.perform_search = True

    if st.session_state.perform_search:
